...
var hotkeys = require('simple-hotkeys')();
...
class Editor {
	...
	constructor() {
		...
		var hot = {
			save: (event) => { this.save(); event.preventDefault(); },
			preview: (event) => { preview.quick(); event.preventDefault(); },
			find: (event) => { search.quick(); event.preventDefault(); },
			clear: (event) => { jsConsole.clear(); event.preventDefault(); }
		};
		hotkeys.add('ctrl+s', hot.save).add('cmd+s', hot.save);
		hotkeys.add('ctrl+p', hot.preview).add('cmd+p', hot.preview);
		hotkeys.add('shift+ctrl+f', hot.find).add('shift+cmd+f', hot.find);
		hotkeys.add('ctrl+k', hot.find).add('cmd+k', hot.clear);
...
//	}
	...
	showToolbar(show) {
		...
		if (show) {
			...
			attachToolbarHandler('fa-save', (event) => { this.save(); });
...
class Search {
	...
	constructor() {
		...
		this.view.addEventListener('keydown', (event) => {
			if (event.keyCode === 13) {
				...
				var replace = this.replaceInput.value;
				room.emit('search', {search, replace}, settings.values.name);
			}
		});
	...
	open(options, clientName) {
		if (clientName !== undefined && clientName !== settings.values.name) {
			...
			this.selectModeButton(document.getElementById((replace ? 'mode-replace' : 'mode-search')));
			if (this.headerView) {
			...
			headerView.textContent = `${(replace ? `Replace "${search}" with "${replace}"` : `Found "${search}"`)}: ${options.matchCount} matches in ${results.length} files`;
			if (replace) {
...
class Settings {
	...
	getDevice(forceDeviceTypeByName = false) {
		var devices = [{diam: 900, type: 'mobile', touch: true}, {diam: 1500, type: 'tablet', touch: true}, {diam: Number.MAX_VALUE, type: 'television', touch: true}, {diam: 1500, type: 'laptop', touch: false}, {diam: 2300, type: 'desktop', touch: false}, {diam: Number.MAX_VALUE, type: 'television', touch: false}];
